...
For example, for an income of 50,000\$, the predicted value for life satisfaction is 3.37. Of course, it's not because you have an income of 50,000\$ that your life satisfaction is 3.37. That's why there's error $ϵ$ in the model. (*This is made up data. The true relation would not be as clear.*)
</div>
...
<div style="font-size: 30px;">
The error (or the residual) is the distance between the prediction (i.e. the line of best fit) and the true observed value. The residuals are the distance measure by the black lines.
</div>
...
For multiple linear regression, we can plot the marginal effect, that is the average effect, when one holds the non-focal variables constant.
