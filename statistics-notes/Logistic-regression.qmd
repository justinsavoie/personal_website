...
The logit function, or the log of the odds is defined as $$logit\:p(x)=\ln \left({\frac {p(x)}{1-p(x)}}\right)=\beta _{0}+\beta _{1}x$$
...
```{r}
summary(fit<-glm(vs~mpg+hp,mtcars,family='binomial'))
```
...
cbind(coef(fit),optim(pars_init, to_optim, y = y, X = X,method = "BFGS")$par)
```
...
cbind(coef(summary(fit_s))[,1:2],fit_optim$par,se_beta_hat)
...
cbind(coef(summary(fit_s))[,1:2],nr$coefficients,nr$std_error)
